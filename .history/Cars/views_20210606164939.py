...
def comm_details(request,comm_id):
    single_post = get_object_or_404(Blog, id=comm_id)
    comments = Comment.objects.filter(id=comm_id).order_by('-timestamp')
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            comment.save()
            return redirect('htmls:comm_details', id=single_post.id)
        #  single_post = {'form': form, 'most_recent': most_recent,}
    else:
        form = CommentForm()
    return render(request, 'htmls/comm_details.html', {'comm':comments, 'form':form, 'sipst':single_post})
