...
def sidebar(request, side_id):
    blg = get_object_or_404(Blog, id=side_id)
    comments = blg.comments.filter(active = True)
    new_comment = None
    ...
    return render(request, 'htmls/sidebar.html', {'comments':comments, 'new_comment':new_comment, 'comment_form':comment_form})
